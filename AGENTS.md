...
Smart Home is a prototype application for people who need to connect and
control smart electronic devices from one product surface. The exact client
platform, supported device protocols, and hardware integrations are not chosen
yet.
...
Treat the supported platform, device category, communication protocol, and
production-grade security or provisioning as open product decisions until the
user selects them. Track the agreed goal in plans and final answers. At
completion, report what was implemented against each criterion and list
remaining gaps as blockers.
...
- Outputs/evidence/build artifacts: `app/build/`
